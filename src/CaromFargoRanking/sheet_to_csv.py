...
from scripts.sheet_api_player_ranking import (get_rating_list, get_ranking_list)
...
if __name__ == "__main__":
    ...
    try:
        ranking = get_ranking_list(spread_sheet)
    except Exception as e:
    ...
    if 1 < len(ranking):
        try:
            with open(config.RATING_LIST_CSV_FILE_PATH, mode="w", encoding="utf-8", newline='') as file:
